...
def compute_candidates(place= "Delhi, India",
    ...
    # convert to centroids and restrict to area polygon
...
with left_col:
    ...
    if st.session_state.candidates_geojson and st.session_state.candidates_geojson.get("features"):
...
with right_col:
    ...
    # ensure pre_lat/pre_lon are either numeric strings or null
    pre_lat_js = json.dumps(pre_lat if pre_lat and pre_lat.strip() else None)
    pre_lon_js = json.dumps(pre_lon if pre_lon and pre_lon.strip() else None)
    ...
    # embed the HTML doc. use scrolling True if you want iframe scrollbars
    st_html(html_doc, height=820, scrolling=True)
